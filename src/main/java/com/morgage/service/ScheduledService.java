...
//    @Scheduled(fixedDelay = 6 * 60 * 1000)
    public void createNotificationForLateTransaction() throws ParseException {
        ...
        for (Map.Entry entry : shopCountNotification.entrySet()) {
            String count = entry.getValue().toString();
            int shopId = Integer.parseInt(entry.getKey().toString());
            notificationService.createNotification(count + env.getProperty("notification.shop.late"), Const.NOTIFICATION_TYPE.SYSTEM_SHOP, null, shopService.getAccountIdByShopId(shopId), null);
        }
        shopCountNotification.clear();
    }
...
////                transactionService.setTransactionStatus(transaction, Const.TRANSACTION_STATUS.LATE);
//                notificationService.createNotification(transaction.getItemName() + env.getProperty("notification.late.begin") + lateDays + env.getProperty("notification.late.end"), Const.NOTIFICATION_TYPE.SYSTEM_PAWNER, null, pawnerService.getAccountIdFromPawnerId(transaction.getPawnerId()), transaction.getId());
//                if (shopCountNotification.containsKey(transaction.getShopId())) {
//                    shopCountNotification.put(transaction.getShopId(), shopCountNotification.get(transaction.getShopId()) + 1);
//                } else {
//                    shopCountNotification.put(transaction.getShopId(), 1);
...
//                notificationService.createNotification(transaction.getItemName() + env.getProperty("notification.overDue"), Const.NOTIFICATION_TYPE.SYSTEM_PAWNER, null, pawnerService.getAccountIdFromPawnerId(transaction.getPawnerId()), transaction.getId());
